...
epmc_search <- function(query = NULL, limit = 25) {
  if(is.null(query)) stop("No query provided")
  path <- paste0("search/query=", query)
  doc <- rebi_GET(path = path)
  hits <- rebi_hits(doc)
  if(hits == 0) warning(sprintf("There are no citations matching your query: %s", query))
  if(hits <= 25) {
...
rebi_GET <- function(path = NULL, ...) {
  ...
  # load xml into r
  doc<- rebi_parse(req)
  if(!exists("doc")) stop("No xml to parse", call. = FALSE)
  doc
}
...
parse_ftxt <- function(ext_id = NULL, xp = NULL) {
  if(is.null(ext_id)) stop("No ext_id provided")
  path <- sprintf("%s/fullTextXML", ext_id)
  doc <- rebi_GET(path = path)
  
  if(is.null(xp)) stop("No xpath expression provided")
